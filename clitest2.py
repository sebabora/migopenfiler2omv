...
@click.argument('ldifPath', required=True, type=click.Path(exists=True)) 
@click.option('--white-list', '-uwl', 'whiteList', type=click.Path(exists=True), help='list of  ')
@click.option('--black-list', '-ubl', 'blackList', type=click.Path(exists=True), help='list of user excluded from migration')
@click.option('--output-file', '-uo', 'outputFile', type=click.Path(exists=True), help='Parsed user output file')
...
@click.pass_context
def migrateData(ctx, src, dst, dry_run):
